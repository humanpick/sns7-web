...
st.markdown("""
    ...
    /* 3. 그래프 우측 상단 '데이터 표시', '전체화면' 아이콘 완전 박멸 */
...
if st.session_state["authentication_status"] == False:
    ...
    # [실시간 이름 동기화] DB에서 무조건 최신 실명 가져오기
    try:
    ...
    if user_role == 'admin':
        ...
        with tab1:
            with st.form("new_form"):
                ...
                with col2:
                    ...
                    c_sales = st.number_input("월매출(만원)", 0, 50000, step=100)
                c_comment = st.text_area("센터장님 코멘트")
        ...
        with tab2:
            if not all_df.empty:
                ...
                if edit_cid != "선택":
                    c_records = all_df[all_df['client_id'] == edit_cid]
                    edit_time = st.selectbox("수정할 날짜", c_records['created_at'].tolist())
                    target = c_records[c_records['created_at'] == edit_time].iloc[0]
    ...
    # ==========================================
        ...
        try:
            ...
            if res.data:
                ...
                st.info(latest.get('strategy_comment', "세부 전략 수립 중입니다."))
