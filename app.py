...
def fetch_users():
    try:
        ...
        # 💡 [안전장치] credentials['usernames']는 반드시 딕셔너리여야 함
        creds = {'usernames': {}}
        if response.data:
...
if st.session_state.get("authentication_status"):
    ...
    # 💡 [안전장치] 유저 정보를 꺼낼 때 .get()을 사용하여 데이터 추출 시 에러를 막습니다.
            ...
            with st.form("user_reg_form"):
                ...
                if st.form_submit_button("고객 정보 저장 및 등록"):
                    if not reg_id or not reg_pw or not reg_name:
                        ...
                        try:
                            # 💡 [Hasher.init 에러 해결 포인트] 
                            # Hasher() 안에 인수를 넣지 않고, 클래스 메서드 hash_passwords를 사용합니다.
                            # 이 방식이 최신 버전에서 요구하는 가장 안정적인 암호화 방식입니다.
        ...
        try:
            ...
            if res.data:
                ...
                with col1:
                    ...
                    chart1 = alt.layer(base.mark_line(color='#ff4b4b', size=3), base.mark_circle(size=150, color='#ff4b4b')).properties(height=350)
                ...
                with col2:
                    ...
                    chart2 = alt.layer(base_s.mark_line(color='#0068c9', size=3), base_s.mark_circle(size=150, color='#0068c9')).properties(height=350)
