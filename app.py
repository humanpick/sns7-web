...
def fetch_users():
    try:
        ...
        # 💡 [핵심] 리스트가 아닌 'usernames'라는 이름표를 가진 딕셔너리로 강제 구조화
        creds = {'usernames': {}}
        if response.data:
            for user in response.data:
                u_id = str(user['username']) # ID를 문자열로 강제 변환
...
if st.session_state.get("authentication_status"):
    ...
    # 💡 에러 방지를 위한 안전한 딕셔너리 접근법
    user_dict = credentials.get('usernames', {}).get(username, {})
    ...
    if user_role == 'admin':
        ...
        with tab1:
            ...
            if not user_list:
                ...
                selected_client = st.selectbox("고객 선택", user_list, 
                ...
                with st.form("data_input_form", clear_on_submit=True):
                    ...
                    with col2:
                        new_sales = st.number_input("월 매출 (만원)", min_value=0, value=1500)
        ...
        with tab2:
                ...
                if st.form_submit_button("고객 등록 및 ID 생성"):
                    if not reg_name or not reg_pw or len(reg_phone) < 4:
                        ...
                        try:
                            ...
                            st.success(f"✅ 등록 완료! 아이디: {generated_id}")
                            st.rerun()
                        except Exception as e:
                            st.error(f"등록 실패: {e}")
            ...
            cl_list = [{"ID": k, "이름": v['name']} for k, v in credentials['usernames'].items() if v['role'] != 'admin']
            if cl_list:
                st.table(pd.DataFrame(cl_list))
