...
# ==========================================
# 2. 데이터베이스 및 인증 구조 (에러 방어 핵심)
# ==========================================
def fetch_users():
    try:
        response = supabase.table('users').select('*').execute()
        # 💡 [안전장치] credentials['usernames']는 반드시 '딕셔너리'여야 합니다.
        creds = {'usernames': {}}
        if response.data:
            for user in response.data:
                # DB의 모든 데이터를 문자열로 안전하게 변환하여 매핑
...
if st.session_state.get("authentication_status"):
    ...
    # 💡 [안전장치] 유저 정보를 가져올 때 .get()을 사용하여 에러를 원천 차단합니다.
    ...
    if user_role == 'admin':
        ...
        with tab1:
            ...
            if not viewer_list:
                ...
                selected_client = st.selectbox("데이터 입력 대상을 선택하세요", viewer_list, 
                ...
                with st.form("data_input_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("데이터 저장"):
                        ...
                        st.success("데이터 저장 및 리포트 발행이 완료되었습니다!")
                        st.rerun()

        # [Tab 2: 고객 등록 및 관리]
        with tab2:
            st.subheader("신규 고객 계정 생성")
            # 💡 [업데이트] 요청하신 순서대로 ID 직접 입력 폼을 구성했습니다.
            with st.form("user_reg_form"):
                reg_id = st.text_input("1. ID (영문/숫자 권장)")
                reg_pw = st.text_input("2. 비밀번호 설정", type="password")
                reg_name = st.text_input("3. 고객 성함")
                reg_phone = st.text_input("4. 휴대폰 번호 (숫자만 입력)")
                
                if st.form_submit_button("고객 정보 저장 및 등록"):
                    if not reg_id or not reg_pw or not reg_name:
                        st.warning("ID, 비밀번호, 성함은 필수 입력 항목입니다.")
                    else:
                        try:
                            # 💡 Hasher 에러 완전 박멸: 최신 표준 암호화 방식 적용
                            ...
                            st.success(f"✅ {reg_name} 대표님(ID: {reg_id}) 등록 완료!")
                            st.rerun()
                        except Exception as e:
                            st.error(f"등록 실패: {e} (ID 중복 여부를 확인하세요)")
            
            st.divider()
            st.subheader("현재 등록된 고객 리스트")
            client_display = [{"ID": k, "이름": v.get('name'), "연락처": v.get('phone')} 
    ...
    # ------------------------------------------
        ...
        try:
            ...
            if res.data:
                ...
                # 수치 정제 및 억 단위 스케일링 (그래프 무결성 프로토콜)
